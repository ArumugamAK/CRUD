...
app.get('/',async (req,res)=>{
    ...
    let message="";
    let edit_id=req.query.edit_id, edit_book;
    if(edit_id){
...
app.post('/store_book',async (req,res)=>{
    let database = await dbo.getDatabase();
    const collection = database.collection('books');
    let book = { title: req.body.title,description : req.body.description };
...
app.post('/update_book/:edit_id',async (req,res)=>{
    let database = await dbo.getDatabase();
    const collection = database.collection('books');
    let book = { title: req.body.title, author: req.body.description  };
